...
import { UDocument, localDocument } from '../document'
import { HoverDocumentGenerator } from '../utils/document-generator'
import { toKebabCase } from '../utils'
import { ExtensionConfigutation, ExtensionVersion } from '..'
import { TagObject } from '.'

export class UViewHoverProvier implements HoverProvider {
  ...
  provideHover(document: TextDocument, position: Position, token: CancellationToken): ProviderResult<Hover> {
    ...
    if (!/^[U|u]-/.test(tag?.text || '')) {
      // 如果不是uview的标签(U|u-开头) 则返回 null 表示没有hover
      return null
    }
  ...
  getHoverInstance(tag: TagObject | undefined, attr: string, range: Range) {
    const config = workspace.getConfiguration().get<ExtensionConfigutation>('uview-helper')
    const version = config?.version || ExtensionVersion.uView2

    const kebabCaseTag = toKebabCase(tag?.text)
    const kebabCaseAttr = toKebabCase(attr)

    return this.createHoverInstance(version, kebabCaseTag, kebabCaseAttr, range)
  }
   ...
   * @param range 范围
   */
  createHoverInstance(version: ExtensionVersion, tag: string, attr: string, range: Range): null | Hover {
    let document: Record<string, any> = localDocument[version]
    if (tag === attr) {
      attr = ''
    }
    if (Object.prototype.hasOwnProperty.call(document, tag)) {
      ...
      Object.keys(tagDocument).forEach((key: string) => {
        const hoverMarkdownString: MarkdownString = HoverDocumentGenerator.getInstance().generate<UDocument>(tagDocument, key, tag, attr, version)

    ...
    if (/\.md$/.test(doc)) {
      ...
      readLine.on('line', (line) => {
        ...
        if (line.includes('Attributes')) {
          filename = 'u-' + line.replace(/#/g, '').replace('Attributes', '').trim().replace(/ /g, '-').toLocaleLowerCase()
          if (filename.length === 3) {
            filename = 'u-' + doc.substr(0, doc.length - 3)
          }
          flag = 'attributes'
          skip = 2
          return
        ...
        if (line.includes('Events') && line.includes('###')) {
          filename = 'u-' + line.replace(/#/g, '').replace('Events', '').trim().replace(/ /g, '-').toLocaleLowerCase()
          if (filename.length === 3) {
            filename = 'u-' + doc.substr(0, doc.length - 3)
          }
          flag = 'events'
        ...
        if (line.includes('Methods')) {
          filename = 'u-' + line.replace(/#/g, '').replace('Methods', '').trim().replace(/ /g, '-').toLocaleLowerCase()
          if (filename.length === 3) {
            filename = 'u-' + doc.substr(0, doc.length - 3)
          }
          flag = 'methods'
        ...
        if (line.includes('Scoped Slot')) {
          filename = 'u-' + line.replace(/#/g, '').replace('Scoped Slot', '').trim().replace(/ /g, '-').toLocaleLowerCase()
          if (filename.length === 3) {
            filename = 'u-' + doc.substr(0, doc.length - 3)
          }
          flag = 'scopedSlots'
        ...
        if (line.includes('Slot')) {
          filename = 'u-' + line.replace(/#/g, '').replace('Slots', '').replace('Slot', '').trim().replace(/ /g, '-').toLocaleLowerCase()
          if (filename.length === 3) {
            filename = 'u-' + doc.substr(0, doc.length - 3)
          }
          flag = 'slots'
      ...
      readLine.on('close', () => {
        Object.keys(document).forEach((key) => {
          ...
          outStream.write(`import { UDocument } from "@/document";` + os.EOL)
          ...
          // 写入尾部
          outStream.write(os.EOL + `export const document: UDocument = {`)
          if (document[key].attributes) {
...
const createIndex = (dir) => {
  console.debug('createIndex-dir', dir)

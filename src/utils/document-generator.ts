...
export class HoverDocumentGenerator {
   ...
   */
  private generateAttribute<T extends BaseDocument>(document: T, tag: string, attribute: string, version: string): MarkdownString {
   ...
   * @memberof HoverDocumentGenerator
   */
  private generateMethods<T extends BaseDocument>(document: T, tag: string, attribute: string, version: string): MarkdownString {
    let isUndefined: boolean = true // 标记是否具有文档
    let markdownString: MarkdownString = new MarkdownString('', true)
   ...
   * @memberof HoverDocumentGenerator
   */
  private generateEvents<T extends BaseDocument>(document: T, tag: string, attribute: string, version: string): MarkdownString {
    let isUndefined: boolean = true // 标记是否具有文档
    let markdownString: MarkdownString = new MarkdownString('', true)
   ...
   * @memberof HoverDocumentGenerator
   */
  private generateSlots<T extends BaseDocument>(document: T, tag: string, attribute: string, version: string): MarkdownString {
    let isUndefined: boolean = true // 标记是否具有文档
    let markdownString: MarkdownString = new MarkdownString('', true)
   ...
   * @memberof HoverDocumentGenerator
   */
  private generateScopedSlots<T extends BaseDocument>(document: T, tag: string, attribute: string, version: string): MarkdownString {
    let isUndefined: boolean = true // 标记是否具有文档
    let markdownString: MarkdownString = new MarkdownString('', true)
   ...
   * @memberof HoverDocumentGenerator
   */
  private generateOther<T extends BaseDocument>(document: T, tag: string, attribute: string, version: string): MarkdownString {
    let isUndefined: boolean = true // 标记是否具有文档
    let markdownString: MarkdownString = new MarkdownString('', true)
   ...
   * @param {ExtensionVersion} version 版本
   * @return {*}  {MarkdownString}
   * @memberof HoverDocumentGenerator
   */
  generate<T extends BaseDocument>(document: T, key: string, tag: string, attr: string, version: ExtensionVersion): MarkdownString {
    let markdownString: MarkdownString = new MarkdownString('')
    switch (key) {
      case 'attributes':
        markdownString = this.generateAttribute<T>(document, tag, attr, version)
        break
      case 'methods':
        markdownString = this.generateMethods<T>(document, tag, attr, version)
        break
      case 'events':
        markdownString = this.generateEvents<T>(document, tag, attr, version)
        break
      case 'slots':
        markdownString = this.generateSlots<T>(document, tag, attr, version)
        break
      case 'scopedSlots':
        markdownString = this.generateScopedSlots<T>(document, tag, attr, version)
        break
      default:
        // 生成其他文档时 属性为key
        markdownString = this.generateOther<T>(document, tag, key, version)
    }
    return markdownString
  }
